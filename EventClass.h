...
//Programmer: Andrew Morgan, Melissa Mattwig
//Date: November 2020
...
class EventClass
{
  ...
  public:
    ...
    //An overloaded operator to allow the less than operator to be used in a 
    //standard way
    bool operator<(const EventClass &rhs) const
    {
      return this->timeOccurs < rhs.timeOccurs;
    }
    
    //An overloaded operator to allow the greater than or equal to operater to 
    //be used in a standard way 
    bool operator>=(const EventClass &rhs) const
    {
      return this->timeOccurs >= rhs.timeOccurs;
    }

    //An overloaded operator to allow the equal to comparison operator to be
